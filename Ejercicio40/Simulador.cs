...
namespace Ejercicio40
{
    public partial class Sim : MetroForm
    {
        ...
        private double a_rand;
        private TimeSpan? c_tiempoEntreLLeg, c_proxLlegada;
        private bool checkInicio;
        private string evento, atencion;
        ...
        public void iniciarSim()
        {
            ...
            var c_desv = string.IsNullOrWhiteSpace(txDc.Text) || isChecked
                ? 1
                : double.Parse(txDc.Text.Replace('.', ','));


            //Creo los Servidores(nombre,media,desviación,indice de evento asociado,personal libre)
            var insumos = new Servidor("Insumos",
                string.IsNullOrWhiteSpace(txMi.Text) || isChecked ? 3 : double.Parse(txMi.Text),
                string.IsNullOrWhiteSpace(txDi.Text) || isChecked ? 1 : double.Parse(txDi.Text.Replace('.', ',')), 2,
                1);
            var telefono = new Servidor("Telefono",
                string.IsNullOrWhiteSpace(txMl.Text) || isChecked ? 1 : double.Parse(txMl.Text),
                string.IsNullOrWhiteSpace(txDl.Text) || isChecked ? 0.5 : double.Parse(txDl.Text.Replace('.', ',')), 3,
                0);
            var presupuesto = new Servidor("Presupuesto",
                string.IsNullOrWhiteSpace(txMp.Text) || isChecked ? 7 : double.Parse(txMp.Text),
                string.IsNullOrWhiteSpace(txDp.Text) || isChecked ? 2 : double.Parse(txDp.Text.Replace('.', ',')), 4,
                3);
            var servTecnico = new Servidor("Servicio Tecnico",
                string.IsNullOrWhiteSpace(txMs.Text) || isChecked ? 3 : double.Parse(txMs.Text),
                string.IsNullOrWhiteSpace(txDs.Text) || isChecked ? 1 : double.Parse(txDs.Text.Replace('.', ',')), 5,
                2);
            ...
            telefono.tiempoConstanteAsociado = string.IsNullOrWhiteSpace(txL.Text) || isChecked
                ? TimeSpan.FromMinutes(5)
                : TimeSpan.FromMinutes(double.Parse(txL.Text.Replace('.', ',')));
            ...
            while (reloj.Value.TotalMinutes < iteraciones)
            {
                switch (indiceEvento)
                {
                    ...
                    case 1:
                        ...
                        if (a_rand < 0.55)
                        {
                            ...
                            if (presupuesto.estado == "Libre")
                            {
                                ...
                                presupuesto.tiempoAtencion =
                                    TimeSpan.FromMinutes(proxEventoNormal(presupuesto.media, presupuesto.desv, rnd));
                            ...
                            else
                            {
                                ...
                                if (presupuesto.personalLibre > 0)
                                {
                                    if (presupuesto.finAtencion == null)
                                    {
                                        presupuesto.estado = "Ocupado";
                                        presupuesto.tiempoAtencion =
                                            TimeSpan.FromMinutes(proxEventoNormal(presupuesto.media, presupuesto.desv,
                                                rnd));
                                        presupuesto.finAtencion = reloj + presupuesto.tiempoAtencion;
                                        presupuesto.personalLibre--;
                                    }
                        ...
                        else if (0.55 < a_rand && a_rand < 0.95)
                        {
                            ...
                            if (insumos.estado == "Libre" && insumos.cola.Equals(0))
                            {
                                ...
                                insumos.tiempoAtencion =
                                    TimeSpan.FromMinutes(proxEventoNormal(insumos.media, insumos.desv, rnd));
                        ...
                        else
                        {
                            ...
                            if (servTecnico.estado == "Libre")
                            {
                                ...
                                servTecnico.tiempoAtencion =
                                    TimeSpan.FromMinutes(proxEventoNormal(servTecnico.media, servTecnico.desv, rnd));
                            ...
                            else
                            {
                                ...
                                {
                                    servTecnico.tiempoAtencion =
                                        TimeSpan.FromMinutes(proxEventoNormal(servTecnico.media, servTecnico.desv,
                                            rnd));
                                    servTecnico.finAtencion = reloj + servTecnico.tiempoAtencion;
                                    servTecnico.personalLibre--;
                                }
                    ...
                    case 6:
                        atencion = null;
                        telefono.tiempoAtencion =
                            TimeSpan.FromMinutes(proxEventoNormal(telefono.media, telefono.desv, rnd));
                ...
                var listaProximos = new List<TimeSpan?>();
                ...
                var listServidores = new List<Servidor>();
                ...
                gvSimulacion.Rows.Add(reloj, evento, c_tiempoEntreLLeg, c_proxLlegada, atencion,
                    insumos.estado, insumos.tiempoAtencion, insumos.finAtencion, insumos.personalLibre,
                    insumos.inicioOcio, insumos.cola,
                    telefono.estado, telefono.tiempoAtencion, telefono.finAtencion,
                    presupuesto.estado, presupuesto.tiempoAtencion, presupuesto.finAtencion, presupuesto2_atencion,
                    presupuesto.proximoEventoAsociado2, presupuesto3_atencion, presupuesto.proximoEventoAsociado3,
                    presupuesto.personalLibre, presupuesto.inicioOcio, presupuesto.cola,
                    servTecnico.estado, servTecnico.tiempoAtencion, servTecnico.finAtencion, servTecnico2_atencion,
                    servTecnico.proximoEventoAsociado2, servTecnico.personalLibre, servTecnico.inicioOcio,
                    servTecnico.cola);
                ...
                if (reloj.Value.TotalMinutes >= iteraciones)
                    foreach (var s in listServidores)
                        if (s.inicioOcio != null)
                            s.ocioTotal += reloj - s.inicioOcio;
            }

            var i_colamax = gvSimulacion.Rows.Cast<DataGridViewRow>()
                .Max(r => Convert.ToInt32(r.Cells["I_cola"].Value));
            var p_colamax = gvSimulacion.Rows.Cast<DataGridViewRow>()
                .Max(r => Convert.ToInt32(r.Cells["P_cola"].Value));
            var st_colamax = gvSimulacion.Rows.Cast<DataGridViewRow>()
        ...
        private void graficar(TimeSpan? r, TimeSpan? ins, TimeSpan? pr, TimeSpan? sv)
        {
            ...
            grTI.Series["Insumos"].ChartType = SeriesChartType.Doughnut;
            grTP.Series["Presupuesto"].ChartType = SeriesChartType.Doughnut;
            grTS.Series["Servicio tecnico"].ChartType = SeriesChartType.Doughnut;
        ...
        {
            var minimo = list.Where(x => x != null).Min(s => s.Value.TotalMinutes);
            return minimo;
        }
        ...
        public int proxIndiceEvento(TimeSpan? reloj, TimeSpan? llegadaCli, List<Servidor> listServ)
        {
            ...
            if (reloj.Equals(llegadaCli))
                return proxEvento = 1;
            foreach (var s in listServ)
            {
                if (s.proximoEventoAsociado2 != null)
                    if (reloj.Equals(s.proximoEventoAsociado2))
                        return proxEvento = s.indiceEvento2;
                if (s.proximoEventoAsociado3 != null)
                    if (reloj.Equals(s.proximoEventoAsociado3))
                        return proxEvento = s.indiceEvento3;
        ...
        private string proxNombreEvento(int indice, List<Servidor> listServ)
        {
            var proxNombreEvento = "";
        ...
        private double proxEventoNormal(double media, double desv, Random rnd)
        {
            //Box-Muller
            var x1 = rnd.NextDouble();
            var x2 = rnd.NextDouble();
            var rnd2 = Math.Cos(2 * pi * x2) * Math.Sqrt(-2 * Math.Log(x1));

            var random = media + desv * rnd2;
            return random;
        }
        ...
        private TimeSpan? actualizarProximoLLamado(TimeSpan? r, Servidor t)
        {
            if (t.proximoEventoAsociado2 <= reloj) t.proximoEventoAsociado2 += t.tiempoConstanteAsociado;
            return t.proximoEventoAsociado2;
        }

        private void mostrarColasMaximas(int i, int p, int s)
        {
            lbcM1.Visible = true;
            lbcM2.Visible = true;
            lbcM3.Visible = true;
            ...
            lbColaMI.ForeColor = Color.FromArgb(68, 138, (int) 255);
        ...
        private void txMinutosSim_Validating(object sender, CancelEventArgs e)
        {
            ...
            if (!int.TryParse(txMinutosSim.Text, out validador))
            {
                e.Cancel = true;
                txMinutosSim.Focus();
                errorProvider.SetError(txMinutosSim,
                    "Ingrese un valor numerico entero para la cantidad expresada en minutos.");
                return;
            }
            ...
            if (validador <= 0 || validador > 87600)
            {
                ...
                txMinutosSim.Focus();
                errorProvider.SetError(txMinutosSim,
                    "Ingrese una cantidad menor a 87600 minutos (simulación equivalente a 2 meses completos sin parar).");
            }
            else
            {
                e.Cancel = false;
            ...
            if (!isChecked)
                if (!string.IsNullOrWhiteSpace(txMc.Text))
                {
                    if (!int.TryParse(txMc.Text, out validador))
                    {
                        e.Cancel = true;
                        txMc.Focus();
                        errorProvider.SetError(txMc,
                            "Ingrese un valor numérico entero para la cantidad expresada en minutos.");
                        return;
                    }
                    ...
                    if (validador <= 0)
                    {
                        ...
                        txMc.Focus();
                        errorProvider.SetError(txMc,
                            "Ingrese una cantidad positiva expresada en minutos, o vacío para usar valor original.");
                        return;
                    }
                }
            ...
            if (!isChecked)
                if (!string.IsNullOrWhiteSpace(txDc.Text))
                {
                    if (!double.TryParse(txDc.Text, out validadorD))
                    {
                        e.Cancel = true;
                        txDc.Focus();
                        errorProvider.SetError(txDc,
                            "Ingrese un valor numerico para la cantidad expresada en minutos.");
                        return;
                    }
